...
pub struct ReviewCheck {
    ...
    /// Binaries removed from `PATH` for the required-absent execution variant.
...
pub enum ReviewExecutionVariant {
    ...
    Normal,
    /// Check executed with its required-absent binaries removed from `PATH`.
    RequiredAbsent,
}
...
/// Presence observation for one configured optional or required-absent binary.
#[derive(Clone, Copy, Debug, Deserialize, Eq, JsonSchema, PartialEq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum ReviewBinaryPresence {
    /// Binary resolved in the execution profile.
    Present,
    /// Binary did not resolve in the execution profile.
    Absent,
}
...
impl Validate for ReviewBinaryObservation {
    fn validate(&self) -> Result<(), ValidationError> {
        match (
            ...
            (ReviewBinaryPresence::Absent, None, None) => Ok(()),
            _ => Err(ValidationError::new(
                "presence",
                ValidationCode::Inconsistent,
                "binary presence must match executable path and digest fields",
            )),
        }
    }
}
...
pub struct ReviewEnvironmentRecord {
    ...
    /// AGSV version, checkout identity, PATH profile, the digest of expanded
    /// declared child values, and safe locale values. This must never contain
    /// an ambient environment dump or the expanded declared values themselves.
    ...
    /// Available observations for optional and required-absent binaries.
    #[schemars(length(max = MAX_REVIEW_ITEMS))]
    pub binary_observations: Vec<ReviewBinaryObservation>,
    /// Binaries removed from `PATH` for this required-absent profile.
    #[schemars(length(max = MAX_REVIEW_ITEMS))]
    pub required_absent_binaries: BTreeSet<ReviewBinaryId>,
}
    ...
    fn validate_execution_environment(&self) -> Result<(), ValidationError> {
        ...
        for (key, value) in &self.execution_environment {
            ...
            if matches!(key.as_str(), "path_digest" | "declared_values_digest") {
                PayloadDigest::new(value.clone())
                    .map_err(|error| error.at(&format!("execution_environment[{key}]")))?;
            }
        }
        for required_key in [
            "os",
                ...
                .any(|key| key.as_str() == required_key)
            {
                return Err(ValidationError::new(
                    "execution_environment",
                    ValidationCode::Required,
                    "must include os, arch, agsv_version, cwd_identity, and declared_values_digest",
                ));
            }
        }
        if !self
            ...
            ReviewExecutionVariant::RequiredAbsent
                if !self.required_absent_binaries.is_empty()
                    && self.required_absent_binaries.iter().all(|binary_id| {
                        observations.get(binary_id) == Some(&ReviewBinaryPresence::Absent)
                    }) =>
            {
                Ok(())
            }
...
fn validate_declared_environment_key(key: &ReviewEnvironmentKey) -> Result<(), ValidationError> {
    if key.as_str().bytes().enumerate().all(|(index, byte)| {
...
mod tests {
    use super::{
        ...
        ReviewTreeIdentity, ReviewVerificationAttempt, TeamProfileSnapshot,
    };
    use crate::{
        ActorEpoch, ActorId, ActorProfileName, ActorStatus, AssignmentPolicyId, GitSha,
        MAX_REQUEST_TEXT_CHARACTERS, MessageId, PayloadDigest, PolicyRevision, PrimaryEpoch,
        RequestId, ReviewAttemptRecordId, ReviewBinaryId, ReviewCheckId, ReviewEnvironmentId,
        ReviewEnvironmentKey, ReviewSessionId, ReviewToolId, TeamId, TeamProfileName,
        TimestampMillis, Validate, ValidationCode, ValidationUnit, WorkspaceId,
    };
    use std::collections::{BTreeMap, BTreeSet};

...
impl BaseStalenessContext {
    pub(crate) fn observe(
        ...
        let target = observe_target(repository, configured_branch);
        Self {
    ...
    pub(crate) fn request_report(&self, base_sha: &str, candidate_sha: Option<&str>) -> Value {
        ...
        let base_committed_at_ms = match commit_time_ms(&self.repository, base_sha) {
            Ok(value) => value,
            Err(detail) => {
        ...
        if base_sha == head_sha {
            return json!({
                ...
                "overlap": overlap_report(&self.repository, base_sha, head_sha, candidate_sha),
            });
        }

        match is_ancestor(&self.repository, base_sha, head_sha) {
            ...
            Ok(false) => match is_ancestor(&self.repository, head_sha, base_sha) {
                Ok(true) => json!({
                    "state": "base_ahead",
                    "declared_base_sha": base_sha,
                    "base_committed_at_ms": base_committed_at_ms,
    ...
    fn behind_report(
        ...
        let count = match rev_list_count(&self.repository, base_sha, head_sha) {
            Ok(value) => value,
            Err(detail) => {
                return unavailable_report(
                    base_sha,
        ...
        let behind_since_ms =
            match oldest_intervening_commit_ms(&self.repository, base_sha, head_sha) {
                Ok(value) => value,
                Err(detail) => {
                    return unavailable_report(
                        base_sha,
...
fn observe_target(repository: &Path, configured_branch: Option<&str>) -> TargetObservation {
    let (source, branch, configured) = if let Some(branch) = configured_branch {
        ("configured", branch.to_owned(), true)
    } else {
        match git_stdout(repository, ["symbolic-ref", "--quiet", "--short", "HEAD"]) {
    ...
    let committed_at_ms = match commit_time_ms(repository, &head_sha) {
        Ok(value) => value,
        Err(detail) => {
            return TargetObservation::Unavailable {
                state: "unavailable",
...
fn overlap_report(
    ...
    match is_ancestor(repository, base_sha, candidate_sha) {
        Ok(true) => {}
        Ok(false) => {
    ...
    let candidate_paths =
        match candidate_touched_paths(repository, base_sha, candidate_sha, target_sha) {
            Ok(paths) => paths,
            Err(detail) => {
                return json!({
                    "state": "not_comparable",
    ...
    let intervening_paths = match touched_paths(repository, base_sha, target_sha) {
        Ok(paths) => paths,
        Err(detail) => {
            return json!({
                "state": "not_comparable",
...
fn commit_time_ms(repository: &Path, sha: &str) -> Result<u64, String> {
...
fn rev_list_count(repository: &Path, base_sha: &str, target_sha: &str) -> Result<u64, String> {
    let range = format!("{base_sha}..{target_sha}");
    git_stdout(repository, ["rev-list", "--count", range.as_str()])?
        .parse::<u64>()
...
fn oldest_intervening_commit_ms(
    ...
    let range = format!("{base_sha}..{target_sha}");
    let output = git_stdout(repository, ["log", "--format=%ct", range.as_str()])?;
    output
        .lines()
...
fn touched_paths(
    ...
    let range = format!("{base_sha}..{target_sha}");
    touched_paths_from_revisions(repository, [range.as_str()], "-m")
}
    ...
    S: AsRef<OsStr>,
{
    ...
    command.push("--".into());
    let output = git_output(repository, command)?;
...
fn git_stdout<I, S>(repository: &Path, args: I) -> Result<String, String>
where
    I: IntoIterator<Item = S>,
    S: AsRef<OsStr>,
{
...
fn git_output<I, S>(repository: &Path, args: I) -> Result<Output, String>
where
    I: IntoIterator<Item = S>,
    S: AsRef<OsStr>,
{
...
mod tests {
    ...
    fn unborn_primary_branch_is_plainly_not_configured() {
        ...
        run_git(temporary.path(), &["init", "-q"]);
        let context = BaseStalenessContext::observe(temporary.path(), None, 1);

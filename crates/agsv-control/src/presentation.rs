...
mod tests {
    use agsv_core::Supervisor;
    use agsv_protocol::{
        ActorId, ActorProfileName, ActorProfileSnapshot, ActorRole, CapabilityId,
        HUMAN_FACING_PRIMARY_CAPABILITY, PolicyRevision, TeamId, WorkspaceId,
    };
    ...
    fn labels_use_team_order_and_purpose_without_changing_actor_identity() {
        ...
        let mut configured = Supervisor::new(
            WorkspaceId::new("workspace-configured-primary").unwrap(),
            PolicyRevision::INITIAL,
        );
        let configured_id = ActorId::new("research-primary").unwrap();
        configured
            .activate_primary_with_profile(
                configured_id.clone(),
                ActorRole::new("research").unwrap(),
                ActorProfileSnapshot {
                    name: ActorProfileName::new("research-primary").unwrap(),
                    capabilities: [CapabilityId::new(HUMAN_FACING_PRIMARY_CAPABILITY).unwrap()]
                        .into_iter()
                        .collect(),
                },
            )
            .unwrap();
        assert_eq!(
            session_label(&configured, &configured_id, "ignored").unwrap(),
            "agsv:primary"
        );
    }

...
impl ReviewRunner {
    ...
    #[cfg(test)]
    pub(crate) fn set_test_inherited_environment(&mut self, key: &str, value: &str) {
        self.inherited_environment
            .insert(key.to_owned(), value.to_owned());
    }

    #[cfg(test)]
    pub(crate) fn set_test_controlled_path(&mut self, value: String) {
        self.controlled_path = Some(value);
    }
    ...
    pub(crate) fn verify_artifact(
        ...
        let session_component = components.next();
        let evidence_component = components.next();
        if !matches!(session_component, Some(Component::Normal(_)))
            || !matches!(
                evidence_component,
                Some(Component::Normal(directory)) if directory == EVIDENCE_DIRECTORY
            )
        {
            return Err(ControlError::new(
                "review_artifact_path_invalid",
                "durable review output must reside in controller-only evidence storage",
            )
            .with_details(json!({ "source": source, "path": path })));
        }
    ...
    pub(crate) fn evidence_path(&self, session_id: &ReviewSessionId) -> PathBuf {
        self.root.join(session_id.as_str()).join(EVIDENCE_DIRECTORY)
    }
    ...
    pub(crate) fn execute_check(
        ...
        let result_root = self
            .evidence_path(&session.session_id)
    ...
    fn capture_tool_versions(
        ...
        for probe in &session.plan.tool_version_probes {
            ...
            verify_output_matches_artifact(&stdout_path, &captured.stdout)?;
            verify_output_matches_artifact(&stderr_path, &captured.stderr)?;
    ...
    fn sanitized_path(
        ...
        let original = self.controlled_path()?;
        self.sanitized_path_from(session_id, absent, &original)
    }

    // Production uses the controller PATH; tests may inject a complete fixture
    // PATH without mutating process-global state.
    #[allow(clippy::unused_self)]
    fn controlled_path(&self) -> Result<String, ControlError> {
        #[cfg(test)]
        if let Some(path) = &self.controlled_path {
            return Ok(path.clone());
        }
        validated_path()
    }

    fn sanitized_path_from(
        &self,
        session_id: &ReviewSessionId,
        absent: &BTreeSet<ReviewBinaryId>,
        original: &str,
    ...
    fn expanded_environment(
        ...
        let declared = session
            ...
            .map(|(key, value)| {
                let value = if value == "{inherit}" {
                    self.inherited_environment_value(key.as_str())
                        .ok_or_else(|| {
                            ControlError::new(
        ...
        let mut child = BTreeMap::from([
            ...
            (
                "GIT_TEMPLATE_DIR".to_owned(),
                self.root
                    .join(EMPTY_GIT_TEMPLATE_DIRECTORY)
                    .to_string_lossy()
                    .into_owned(),
            ),
        ...
        child.extend(declared);
        Ok((child, declared_values_digest))
    }

    // The receiver carries test-only inherited values; production falls back
    // to the actual controller environment.
    #[allow(clippy::unused_self)]
    fn inherited_environment_value(&self, key: &str) -> Option<String> {
        #[cfg(test)]
        if let Some(value) = self.inherited_environment.get(key) {
...
fn execution_environment(
    ...
    for (source, recorded) in [
        ...
        if let Some(value) = child_environment.get(source) {
            values.insert(
                ReviewEnvironmentKey::new(recorded.to_owned()).map_err(ControlError::protocol)?,
                value.clone(),
            );
        }
    }
...
fn create_output_file(path: &Path) -> Result<File, ControlError> {
    ...
    if path.exists() {
        return Err(ControlError::new(
            ...
            "review output path already exists before controller capture is finalized",
        )
        .with_details(json!({ "path": path })));
    }
    tempfile::tempfile_in(parent)
        .map_err(|error| ControlError::io("create anonymous review output", parent, &error))
}
...
fn capture_bounded_output(
    ...
    Ok(CapturedStream {
        artifact: persist_output_artifact(&mut output, session_root, output_path, truncated)?,
        incomplete,
    })
}
...
fn persist_output_artifact(
    ...
    if !matches!(
        ...
        return Err(ControlError::new(
            "unsafe_review_path",
            "review output must be published beneath controller-only evidence storage",
        )
        .with_details(json!({ "path": output_path })));
    }
    let byte_count = output
        .metadata()
        .map_err(|error| ControlError::io("inspect anonymous review output", output_path, &error))?
        .len();
    let digest = digest_open_file(output, output_path)?;
    output
        .seek(SeekFrom::Start(0))
        .map_err(|error| ControlError::io("rewind anonymous review output", output_path, &error))?;
    reject_symlink(output_path)?;
    let mut persisted = OpenOptions::new()
        .create_new(true)
        .write(true)
        .mode(0o600)
        .open(output_path)
        .map_err(|error| {
            if error.kind() == std::io::ErrorKind::AlreadyExists {
                ControlError::new(
                    "review_output_conflict",
                    "review output path was created outside controller capture",
                )
                .with_details(json!({ "path": output_path }))
            } else {
                ControlError::io("publish review output", output_path, &error)
            }
        })?;
    std::io::copy(output, &mut persisted)
        .map_err(|error| ControlError::io("publish review output", output_path, &error))?;
    persisted
        .sync_all()
        .map_err(|error| ControlError::io("sync published review output", output_path, &error))?;
    fsync_directory(output_path.parent().expect("review output has a parent"))?;
...
fn digest_open_file(file: &mut File, path: &Path) -> Result<PayloadDigest, ControlError> {
    file.seek(SeekFrom::Start(0))
        .map_err(|error| ControlError::io("rewind anonymous review output", path, &error))?;
    let mut hasher = Sha256::new();
    let mut buffer = vec![0_u8; 64 * 1024].into_boxed_slice();
    loop {
        let read = file
            .read(&mut buffer)
            .map_err(|error| ControlError::io("digest anonymous review output", path, &error))?;
        if read == 0 {
            break;
        }
        hasher.update(&buffer[..read]);
...
fn verify_output_matches_artifact(
    path: &Path,
    artifact: &ReviewOutputArtifact,
) -> Result<(), ControlError> {
    let metadata = fs::metadata(path)
        .map_err(|error| ControlError::io("inspect published review output", path, &error))?;
    if metadata.len() != artifact.byte_count || digest_file(path)? != artifact.digest {
        return Err(ControlError::new(
            "review_output_integrity_mismatch",
            "published review output no longer matches the controller-captured bytes",
        )
        .with_details(json!({ "path": path })));
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn neutral_git_command_clears_repository_overrides() {
    ...
    #[test]
    fn required_absent_path_removes_a_fixture_controlled_executable() {
        let temporary = tempfile::tempdir().unwrap();
        let source_bin = temporary.path().join("source-bin");
        std::fs::create_dir(&source_bin).unwrap();
        for name in ["required-tool", "forbidden-tool"] {
            let path = source_bin.join(name);
            std::fs::write(&path, "#!/bin/sh\nexit 0\n").unwrap();
            std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o700)).unwrap();
        }
        let original = std::env::join_paths([source_bin])
            .unwrap()
            .to_string_lossy()
            .into_owned();
        assert!(
            resolve_executable_optional("forbidden-tool", &original)
                .unwrap()
                .is_some()
        );
        ...
        let session_id = ReviewSessionId::new("review-required-absent").unwrap();
        let absent = BTreeSet::from([ReviewBinaryId::new("forbidden-tool").unwrap()]);
        let sanitized = runner
            .sanitized_path_from(&session_id, &absent, &original)
            .unwrap();
        assert!(
            resolve_executable_optional("forbidden-tool", &sanitized)
                .unwrap()
                .is_none()
        );
        assert!(
    ...
    #[test]
    fn checkout_read_only_application_and_verification_are_both_measured() {
        let temporary = tempfile::tempdir().unwrap();
        let checkout = temporary.path().join("checkout");
        let nested = checkout.join("nested");
        std::fs::create_dir_all(&nested).unwrap();
        let tracked = nested.join("tracked.txt");
        std::fs::write(&tracked, "exact tree\n").unwrap();
        std::fs::set_permissions(&checkout, std::fs::Permissions::from_mode(0o755)).unwrap();
        std::fs::set_permissions(&nested, std::fs::Permissions::from_mode(0o775)).unwrap();
        std::fs::set_permissions(&tracked, std::fs::Permissions::from_mode(0o764)).unwrap();

        make_tree_read_only(&checkout).unwrap();
        ensure_tree_read_only(&checkout).unwrap();
        for path in [&checkout, &nested, &tracked] {
            assert_eq!(
        ...
        let writable = ensure_tree_read_only(&checkout).unwrap_err();
        assert_eq!(writable.code, "review_checkout_writable");
    }

    #[test]
    fn resolve_tree_rejects_an_annotated_tag_object_instead_of_its_commit() {
        let temporary = tempfile::tempdir().unwrap();
        let repository = temporary.path().join("repository");
        std::fs::create_dir(&repository).unwrap();
        assert!(git(&repository, &["init", "-q"]).status.success());
        assert!(
            git(&repository, &["config", "user.name", "AGSV Test"])
                .status
                .success()
        );
        assert!(
        ...
        assert!(
            git(
                &repository,
                &["tag", "-a", "review-tag", "-m", "review tag"]
            )
            .status
            .success()
        );
        let tag = git(&repository, &["rev-parse", "refs/tags/review-tag"]);
        assert!(tag.status.success());
        let candidate = GitSha::new(String::from_utf8(tag.stdout).unwrap().trim().to_owned())
            .expect("annotated tag object has a SHA-shaped identity");
        let runner = ReviewRunner {
            repository,
            root: temporary.path().join("reviews"),
            settings: ReviewSettings::default(),
        ...
        let error = runner.resolve_tree(&candidate).unwrap_err();
    ...
    fn bounded_capture_persists_an_exact_prefix_and_marks_truncation() {
        let temporary = tempfile::tempdir().unwrap();
        let path = temporary.path().join("evidence/stdout.bin");
        ...
        let budget_path = temporary.path().join("evidence/budget.bin");
        let control = OutputCaptureControl {

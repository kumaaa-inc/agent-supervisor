...
use agsv_protocol::{
    ...
    ReviewDecision, ReviewEnvironmentRecord, ReviewExecutionVariant, ReviewOutputArtifact,
    ReviewPlan, ReviewProcessContainment, ReviewRecoveryState, ReviewSession, ReviewSessionId,
    ReviewSessionState, ReviewSessionStatus, ReviewVerificationAttempt, Run, Team,
...
// observability summaries, plus immutable closed-team generation records.
...
";
    ...
    #[test]
    #[allow(clippy::too_many_lines)]
    fn literal_schema_10_with_observability_is_preserved_then_schema_11_is_created() {
        let directory = tempfile::tempdir().unwrap();
        let database = directory.path().join("control.sqlite3");
        assert_eq!(CONTROL_SCHEMA_VERSION, 11);
        let prior_schema_version = 10;
        ...
        rejected
            .execute_batch(
                "CREATE TABLE archive_manifest (
                   workspace_id TEXT PRIMARY KEY, commit_count INTEGER NOT NULL,
                   commit_head_sha256 TEXT, delivery_count INTEGER NOT NULL,
                   request_count INTEGER NOT NULL, run_count INTEGER NOT NULL,
                   audit_event_count INTEGER NOT NULL, updated_revision INTEGER NOT NULL,
                   updated_at_ms INTEGER NOT NULL
                 );
                 CREATE TABLE archive_commits (
                   workspace_id TEXT NOT NULL, sequence INTEGER NOT NULL,
                   previous_sha256 TEXT, commit_sha256 TEXT NOT NULL,
                   commit_json TEXT NOT NULL, committed_revision INTEGER NOT NULL,
                   committed_at_ms INTEGER NOT NULL, PRIMARY KEY(workspace_id, sequence)
                 );
                 CREATE TABLE archive_commit_entries (
                   workspace_id TEXT NOT NULL, kind TEXT NOT NULL, key TEXT NOT NULL,
                   commit_sequence INTEGER NOT NULL, entry_ordinal INTEGER NOT NULL,
                   content_sha256 TEXT NOT NULL, PRIMARY KEY(workspace_id, kind, key)
                 );",
            )
            .unwrap();

...
The v0.3 release targets macOS with Herdr, Claude Code, and Codex. See [the
...
each immutable request base is behind that branch and, once a candidate exists,
whether the intervening commits touch the candidate's changed files. When the
setting is absent, AGSV uses the branch attached to the canonical primary
worktree rather than assuming a name; a detached or unborn primary worktree
reports no target. Reporting never fetches, rebases, moves a declared base, or
otherwise remediates the gap.
